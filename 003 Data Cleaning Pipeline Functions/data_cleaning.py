...
# https://github.com/balapriyac/python-basics/blob/main/automate-data-cleaning/useful_snippets.py 
...
def load_dataset(file_path):
    ...
    handlers = {'.csv': pd.read_csv, '.xlsx': pd.read_excel, '.json': pd.read_json, '.parquet': pd.read_parquet}
...
# Data Validation
def validate_data(df, validation_rules:None):
        """
...
def generate_quality_metrics(df, baseline_metrics=None):
    ...
    metrics = {'row_count': len(df), 'missing_values': df.isna().sum().to_dict(),
        'unique_values': df.nunique().to_dict(), 'data_types': df.dtypes.astype(str).to_dict()}
